...
const BalanceForm = () => {
  const [formData, setFormData] = useState({
    items: [{ description: '', amount2023: '', LIABILITIES: '', amount2024: '', ASSETS: '' }],
    subTotal: 0,
    assetTotal: 0, 
    assetTotal: 0,
    place: '',
  ...
  useEffect(() => {
    // Recalculate both subTotal (for liabilities + amounts) and assetTotal (for assets)
    let subTotal = 0;
    let assetTotal = 0;
    
    formData.items.forEach((item) => {
      subTotal += (parseFloat(item.amount2023) || 0) + (parseFloat(item.amount2024) || 0); // Add amounts to subtotal
      assetTotal += (parseFloat(item.amount2023_again) || 0 + (parseFloat(item.amount2024_again))); // Add assets to asset total
    });
  ...
  }, [formData.items]); // Dependency on items to recalculate whenever items change
  ...
  const handleAddItem = () => {
    setFormData({
      ...
      items: [
        ...formData.items,
        { description: '', amount2023: '', expenditure: '', amount2024: '', income: '' },
      ],
    });
  };
  ...
  const handleGeneratePDF = () => {
    ...
    doc.text('BALANCE SHEET AS ON 31ST MARCH 20___	', 105, 20, null, null, 'center');
    ...
    const headers = [
      [
        ...
        { content: 'LIABILITIES', rowSpan: 2 },
        { content: '', rowSpan: 2 },
        { content: '31.03.2024 Amount', rowSpan: 2 },
        { content: '', rowSpan: 2 },
        { content: 'ASSETS', rowSpan: 2 },
        { content: '', rowSpan: 2 },
        { content: '31.03.2024 Amount', rowSpan: 2 },
      ],
    ];
    ...
    // Save PDF
    doc.save(`${formData.societyName}-balancesheet.pdf`);
  };

  return (
    <div className="balance-container">
      <form onSubmit={(e) => e.preventDefault()} className="balance-form">
        <h3 style={{ textAlign: 'center', marginTop: '10px', marginBottom: '40px' }}>
          BALANCE SHEET AS ON 31ST MARCH 20___
        </h3>
